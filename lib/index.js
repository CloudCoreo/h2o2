        ...
        settings.mapUri(request, function (err, uri, headers) {
            ...
            const _sendRequest = function (data) {
                ...
                Wreck.request(request.method, uri, options, function (err, res) {
                    ...
                    if (modifyResponse) {
                        modifyResponse(res, data).then(function (ret) {

                            _sendResponse(ret.response, ttl, ret.data);
                        }).catch(function (err) {

                            var msg = 'Failed request';
                            if (err.message) {
                                msg = err.message;
                            }
            ...
            // Send request
            if (modifyPayload) {
                modifyPayload(request).then(function (payloadAndData) {

                    options.payload = payloadAndData.payload;

                    // DM: Added to enable AWS signing
                    Hoek.merge(options.headers, headers);
